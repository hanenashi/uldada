...
(() => {
  ...
  const addStyle = () => {
    ...
    style.textContent = `
      :root{--uldada-scale:${scale};--uldada-chrome:${Math.min(scale, 1.35)}}
  ...
  const mount = async () => {
    ...
    const [lines, initialScale, initialFormat, initialSize] = await Promise.all([savedLines(), savedScale(), savedFormat(), savedSize()]);
    ...
    document.documentElement.style.setProperty("--uldada-chrome", String(Math.min(initialScale / 100, 1.35)));
    ...
    const applyScale = () => {
      ...
      document.documentElement.style.setProperty("--uldada-chrome", String(Math.min(value / 100, 1.35)));
      saveScale(value);
      if (menu.classList.contains("uldada-open")) place();
    };
    try {
